...
NOVECTORIZE std::unique_ptr<runtime::Query> q12_hyper(Database& db,
   ...
   // --- constants
   const auto relevant_yearmonthnum = types::Integer(199401);
   const auto discount_min = types::Numeric<18, 2>::castString("4.00");
   const auto discount_max = types::Numeric<18, 2>::castString("6.00");
   ...
   tbb::enumerable_thread_specific<runtime::Stack<decltype(ht)::Entry>>
       entries1;
   auto& d = db["date"];
   auto d_yearmonthnum = d["d_yearmonthnum"].data<types::Integer>();
   ...
   auto found = PARALLEL_SELECT(d.nrTuples, entries1, {
      auto& yearmonthnum = d_yearmonthnum[i];
      auto& datekey = d_datekey[i];
      if (yearmonthnum == relevant_yearmonthnum) {
...
//                             Column(date, "d_yearmonthnum"),
//                             Value(&r->yearmonthnum)));
//
//   auto lineorder = Scan("lineorder");
